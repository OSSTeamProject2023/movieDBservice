...
void DB_list::readInfo(DB_list *list, int index){
    ...
    for(int i = 0; i < index; i++) {
        if(list[i].price == -1) continue;
            ...
            cout << "*********************************************************" << endl;
            cout << "*                                                       *" << endl; 
            // window = if(list[i].movie_name.length() > 6)
            // mac = if(list[i].movie_name.length() > 18)
            if(list[i].movie_name.length() > 6) cout << "*\t 영화 제목: " << list[i].movie_name << "\t\t\t*" << endl;
            else cout << "*\t 영화 제목: " << list[i].movie_name << "\t\t\t\t*" << endl;
            cout << "*\t상영일시: " << list[i].date << "\t예매자: " << list[i].name << "\t\t*" << endl;  
